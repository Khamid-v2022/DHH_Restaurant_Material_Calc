...
var reload_table = function(){
    ...
    table = $('.datatable-ajax').DataTable({
        ajax: SITE_URL + 'admin/companies/get_users',
        // "order": [[ 0, "asc" ]]
    }).page.len(50).draw();
...
function save(){
    ...
    $.post(SITE_URL + 'admin/companies/save_company', 
        {
...
function delete_user(id){
    $.post(SITE_URL + 'admin/companies/delete_user/' + id, function(resp){
        resp = JSON.parse(resp)
        if(resp.status){
            reload_table();
        }
...
function reset_password(id){
    $.post(SITE_URL + 'admin/companies/format_password/' + id, function(resp){
        resp = JSON.parse(resp)
        if(resp.status){
            if(resp.status){
                swal({
