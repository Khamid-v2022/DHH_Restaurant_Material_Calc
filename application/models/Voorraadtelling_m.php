...
class Voorraadtelling_m extends MY_Model{
	...
	public function get_stock_list($id){
		...
		INNER JOIN basic_leveranciers b_leve ON leve.leveranciers_id = b_leve.id 
		INNER JOIN basic_locatie b_l ON leve.locatie_id = b_l.id 
		INNER JOIN basic_inkoopcategorien b_i ON leve.inkoopcategorien_id = b_i.id 
		INNER JOIN basic_eenheid b_ei ON leve.eenheid_id = b_ei.id 
		...
		INNER JOIN basic_statiegeld b_s ON leve.statiegeld_id = b_s.id 
		LEFT JOIN (	
			SELECT leve_copy_id, COUNT(id) num FROM voorraadteling_puchase 
	...
	public function get_stock_list_by_location($id, $location_id){
	...
	public function get_stock_histories($leve_copy_id){
		...
		FROM ( 
			...
			WHERE leve_copy_id = " . $leve_copy_id . ") his 
		LEFT JOIN leverancierslijst_copy copy ON his.leve_copy_id = copy.id 
		INNER JOIN basic_statiegeld b_s ON his.statiegeld_id = b_s.id 
		INNER JOIN basic_statiegeld b_s2 ON his.statiegeld_los = b_s2.id 
